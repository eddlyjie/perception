...
def demo_objects(object_perception, object_yolo_names, realsense, transformation=np.eye(4), merge_contours=True, vis=True):
    """
    Detect multiple object types (e.g., bread, bowl, plate) via YOLO + mask,
    estimate grasp poses via PCA, and return their positions + yaw in base frame.
    
    Args:
        object_perception: perception module with yolo_seg, get_pointcloud, etc.
        object_yolo_names (list[str]): list of YOLO class names to detect.
        realsense: wrapper for RGB-D camera
        transformation (4x4): extrinsic camera-to-base transformation matrix
        vis (bool): enable visualization
    
    Returns:
        List of [ [x, y, z], yaw, label ] in base frame for each object.
    """
    # 1. Get frames and intrinsics
    color_image, depth_image = realsense.get_frames()
    K = realsense.K
    ...
    # 2. Detect each object category
    for obj_name in object_yolo_names:
        seg_output = object_perception.yolo_seg(color_image, obj_name, visualize=False)
    ...
    for idx, (contour, obj_label) in enumerate(zip(all_contours, all_labels)):
        ...
        try:
            ...
            center_point = points_mean / 1000.0  # mm → m
            ...
            if vis:
                ...
                # Draw text
                text_x = int(center_px[0]) + 10
                text_y = int(center_px[1]) - 10
                cv2.putText(vis_img, f"{obj_label.capitalize()} {idx+1}", (text_x, text_y), 
                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
                cv2.putText(vis_img, f"Yaw: {np.degrees(yaw):.1f} deg", (text_x, text_y + 20), 
                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1)
                cv2.putText(vis_img, f"Pos: [{base_center[0]:.2f}, {base_center[1]:.2f}, {base_center[2]:.2f}]", 
                            (text_x, text_y + 40), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1)
...
if __name__ == "__main__":
    ...
    if Demo == "all":
        ...
        # return each bowl's [meters in front of base, meters to the left of base, meters above base]
        for i in range(10):
            positions = demo_objects(bread_perception, ["bowl","wooden-block","tennis","box","bread","can","cone","lollipop","ocean-ball","paper-box"], realsense, merge_contours= False, vis=False)
            if i==9:
